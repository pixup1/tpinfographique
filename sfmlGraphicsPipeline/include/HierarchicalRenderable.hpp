 ...
 * - m_globalTransform that defines the transformation with respect to the parent.
 * The compositions of the global transforms up to the root provides the local
 * frame of the instance, in world space. The result of this composition is named
 * the total global transform.
...
public :
     ...
     *
     * Initialize the local and global transforms to identity, the model matrix to
     ...
     * matrices \ref m_globalTransform. This computation is done in this function and should be
     * typically applied before drawing a hierarchical renderable. The result is stored
     * in \ref m_model.
     */
    void updateModelMatrix();

    /** @brief Compute the total global transformation.
     *
     * This function composes recursively the global transformations until
     * it reaches the root of the hierarchy.
     *
     * \return The total global transformation matrix.
     */
    glm::mat4 computeTotalGlobalTransform() const;

    /** \brief Read only access to the global transformation.
     *
     * Allows to read the value of \ref m_globalTransform.
     *
     * \return A read only reference to \ref m_globalTransform, the global transformation matrix.
     */
    const glm::mat4& getGlobalTransform() const;

    /** \brief Set the value of the global transformation.
     *
     * Write to the \ref m_globalTransform matrix. Avoid to have non uniform scale
     * here: prefer to make it in the local transform. Otherwise, you will obtain
     * shearing.
     *
     * \param globalTransform The new global transformation matrix.
     */
...
private:
     ...
     *
     * This transformation is applied after the global transformation to give

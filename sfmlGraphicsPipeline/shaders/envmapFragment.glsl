...
void main()
{
    ...
    vec3 diffuseEnvmap = vec3(texture(diffuseSampler, surfel_normal));

    vec3 bounce = reflect(-surfel_to_camera, surfel_normal);
    vec3 specularEnvmap = vec3(texture(specularSampler, bounce));


    float dw = 0.5 + 0.5 * sin(time);
    float sw = 1-dw;
    // One can see the incoming light as a directional light coming from the normal opposite direction
    // The two following blocks should be equivalent but some numerical precision issue lead to artifacts
    // with specular factor in the first block.
    /*{
        DirectionalLight diffuseEnvmapLight = DirectionalLight(-surfel_normal, vec3(0), diffuseEnvmap, vec3(0));
        tmpColor += dw * computeDirectionalLight(diffuseEnvmapLight, surfel_to_camera);
        DirectionalLight specularEnvmapLight = DirectionalLight(-bounce, vec3(0), vec3(0), specularEnvmap);
        tmpColor += sw * computeDirectionalLight(specularEnvmapLight, surfel_to_camera);
    }*/
    {
        tmpColor += dw * diffuseEnvmap * material.diffuse; 
        tmpColor += sw * specularEnvmap * material.specular;
    }

    // Let's add some Fresnel effect (= more specular when the light hits in a tagential fashion)
    float ct = clamp(dot(bounce, surfel_normal), 0, 1);
    float fresnel_weight = 3; // looks great with 3
    vec3 fresnel = sw * fresnel_weight * pow(clamp(1 - ct, 0, 1), 5) * specularEnvmap * material.specular;
    tmpColor += fresnel;
